...
\newglossaryentry{Document} {
	name=Document,
	description={Un Document è un documento in stile JSON con schema dinamico (MongoDB chiama il formato BSON), all'interno del quale vengono salvati i dati del database. Paragonabile come concetto ad una tabella di un database relazionale}
}

\newglossaryentry{html} {
	name=HTML,
	description={L'HyperText Markup Language (HTML), in informatica è il linguaggio di markup solitamente usato per la formattazione e impaginazione di documenti ipertestuali disponibili nel World Wide Web sotto forma di pagine web}
}

\newglossaryentry{route} {
	name=route,
	description={Con route si riferisce alla definizione di un URI e a come esso risponde ad una specifica richiesta HTTP di un client}
}
...
\newglossaryentry{schema} {
	name=schema,
	description={Lo schema di un database è la struttura o organizzazione logica dei dati in esso contenuti descritta mediante l'uso di un linguaggio formale}
}
