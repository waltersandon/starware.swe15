...
\begin{document}
	...
	\begin{center}
		\begin{longtable}[H]{p{0.12\textwidth} p{0.2\textwidth} p{0.18\textwidth} p{0.39\textwidth}}
			...
			\midrule
			\midrule
			2.0.6 & \AB & 2016-05-11 & Modifica metrica Cost of Poor Quality\\*
			\midrule
			2.0.5 & \AB & 2016-05-06 & Inserita risultati misurazione fase \FC \\*
			\midrule
			2.0.4 & \WS & 2016-05-06 & Inserita la sezione Codifica nel dettaglio delle verifiche tramite analisi e descritto l'esito della \RP\\*
			\midrule
			2.0.3 & \AB & 2016-05-05 & Modifica metrica Cost of Poor Quality\\*
			\midrule
	...
	dove: 
	\begin{itemize}
		\item MSV indica \mgls{milestone schedule variance};
	...
	\[MSV attività\_gruppo [\%] = \frac{\sum_{i=0}^n msv_i}{n}\]
			dove: 
			\begin{itemize}
...
\end{itemize}
 Se $SV < 0$ significa che il progetto sta producendo con maggior velocità rispetto a quanto pianificato, viceversa se negativo.
	...
	\[CoPQ_{generale}[\%]= \frac{(V*0.25) +S}{V+S}\]
	\[CoPQ_{documenti}[\%]= \frac{(sezV*0.25)+sezE}{sezV+sezE}\]
	\[CoPQ_{progettazione}[\%]=\frac{(classiV*0.25)+classiE}{classiV+classiE}\]
	...
	\begin{itemize}
		\item \textbf{Range di Accettazione}: $\leq$ 30\%
		\item \textbf{Range Ottimale}: $\leq$ 10\%
	\end{itemize}
...
\begin{center}
		\begin{longtable}[]{p{0.14\textwidth} p{0.325\textwidth} p{0.5\textwidth} p{0.025\textwidth}}\midrule
			
	\textbf{Id} & \textbf{Metodo} & \textbf{Descrizione} & \textbf{Stato}\\ \midrule\midrule
...
\begin{center}
	\begin{longtable}[]{p{0.175\textwidth} p{0.4\textwidth} p{0.4\textwidth} p{0.025\textwidth}}
		\midrule\midrule
	157.188.TU& admin: UsersList.filterByRole() & deve filtrare la lista dei ruoli& P \\ 
	\midrule
	...
	193.193.TU& public: Home.checkLogged() & should have a method to check if the path is active & P\\ \midrule
	193.193.TU& public: Home.urlPath() & should have a method to check if the path is active & P\\ \midrule
	83.190.TU& public: LogIn.checkPassword() & deve eseguire il controllo sul formato del password, deve eseguire il controllo sul formato del password sbagliato& P \\ \midrule
	83.189.TU& public: LogIn.checkUserName() & deve eseguire il controllo sul formato del userName, deve eseguire il controllo sul formato del userName sbagliato & P\\ \midrule
	83.91.TU& public: LogIn.submit() & deve effetuare login con credenziali giusti, deve segnalare errore di login con credenziali sbagliati& P \\ \midrule
...
\begin{center}
	\begin{longtable}[]{p{0.125\textwidth} p{0.3\textwidth} p{0.5\textwidth} p{0.05\textwidth}}
			\midrule
			\textbf{ID} & \textbf{classe} & \textbf{descrizione} & \textbf{result} \\
			 \midrule
			 \midrule
...
\section{Dettaglio delle verifiche tramite analisi}\TODO{modifica titolo}
	...
	\begin{table}[H]
		\begin{center}
			\begin{tabular}{p{0.4\textwidth} p{0.2\textwidth} p{0.3\textwidth}}
				...
				Requirement Stability  & 100\% & Ottimale \\ \midrule \midrule
			\end{tabular}	
		\end{center}
...
\end{center}
	...
	I valori percentuali indicano la quntità di righe di codice sul torale che rientrano nel range indicato.
		...
		\begin{table}[H]
			\begin{center}
				\begin{tabular}{p{0.28\textwidth} p{0.24\textwidth} p{0.24\textwidth}p{0.24\textwidth}}
					\midrule
						...
						complessità ciclomatica & 0,00\% & 100,00\% & 0,00\% \\\midrule
