...
\midrule
\midrule
1.0.1 & \WS & 2016-05-03  & Inserimento indice documento \\
\midrule
...
\subsection{Akka} 
è un toolkit di strumenti per la costruzione di applicazioni con elevata concorrenza di dati che necessitano di un sistema resiliente per l'invio e la ricezione di messaggi.
...
\item[Express] 
\hfill\\è un \mgls{framework} dell'applicazione web \mgls{node.js} flessibile e leggero che fornisce una serie di funzioni avanzate per le applicazioni web e per dispositivi mobili.
\end{description}
...
\item[HTTP] 
\hfill\\ è un protocollo a livello applicativo usato come principale sistema per la trasmissione d'informazioni sul web ovvero in un'architettura tipica client-server.
...
\item[Mocha] 
\hfill\\ Mocha è un framework di test su \mgls{javascript} ricco di funzionalità, in esecuzione su \mgls{node.js} o nel browser , rende test asincroni semplici  e divertenti .I test sono eseguiti in serie , consentendo il reporting flessibile e preciso. Hosted su \mgls{github}
