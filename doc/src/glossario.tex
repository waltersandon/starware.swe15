...
\midrule
0.1.0 & \NC & 2016-04-09  & Verifica \\
\midrule
0.1.0 & \NC & 2016-04-08  & Revisione e correzione errori ortografici \\
\midrule
...
\item[Client]
\hfill\\nell'architettura client-server il \mgls{client} è il componente che richiede un qualsiasi servizio ad un server; si può riferire all'applicazione o all'\mgls{hardware}.
...
\item[Node.js] 
\hfill\\ è una piattaforma lato server avvolto attorno al linguaggio \mgls{javascript} per la creazione di applicazioni scalabili, utilizzando un modello \mgls{event-driven}. Ovvero si eseguono azioni solo al verificarsi di un evento.

\item[NoSQL] 
\hfill\\ come dice anche il termine questi database non sono basati su \mgls{sql}. Sono performanti nel caso sia necessario salvare una grande quantità di dati, magari usando la scalabilità orizzontale, quando cioè si utilizzano più server dove salvare questi dati e non solamente incrementando la potenza di un singolo server.
\end{description}
...
\item[Request] 
\hfill\\richiesta fatta da un client ad un server, ad esempio \mgls{http}.
...
\hfill\\motore JavaScript \mgls{open-source} sviluppato da \mgls{google} e scritto in C++ che  può essere eseguito stand-alone esempio in server destinati a supportare il framework Node.js.
