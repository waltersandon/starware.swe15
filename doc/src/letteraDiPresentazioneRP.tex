...
\textbf{Oggetto:} partecipazione alla Revisione di Progettazione\\

Egregio Professore Tullio Vardanega,\\
Con la presente, il gruppo \nomeGruppo\ intende comunicarle ufficialmente la partecipazione alla \textbf{Revisione di Progettazione} del prodotto da lei commissionato, denominato:
\begin{center}
	\textbf{\prjL}
\end{center}
...
\begin{itemize}
	\item \AdR\ (\filePath{esterni/AnalisiDeiRequisiti\_v2.0.0.pdf});
	\item \DP\ (\filePath{esterni/DefinizioneDiProdotto\_v1.0.0.pdf});
	\item \GL\ (\filePath{esterni/Glossario\_v1.0.0.pdf});	
	\item \PdP\ (\filePath{esterni/PianoDiPreogetto\_v2.0.0.pdf});
	\item \PdQ\ (\filePath{esterni/PianoDiQualifica\_v2.0.0.pdf});
	\item \ST\ (\filePath{esterni/SpecificaTecnica\_v1.0.0.pdf});
	\item \NdP\ (\filePath{interni/NormeDiProgetto\_v2.0.0.pdf}).
\end{itemize}

Il gruppo \nomeGruppo\ conferma la previsione della data di consegna del prodotto richiesto entro la revisione che si terrà in data \textit{17 giugno 2016}.
...
Cordiali Saluti,\\
\AB \\
\includegraphics[width=6cm]{../img/firmaBonaldo.png}
