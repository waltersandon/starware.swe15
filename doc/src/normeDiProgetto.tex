...
Parallelamente alla progettazione di dettaglio dei componenti \mgls{software} dovranno essere progettati i relativi test di unità che verranno descritti nel \PdQ{}.
...
Per aggiungere un \mgls{package} è necessario eseguire i seguenti passi:
\begin{enumerate}
	\item eseguire l'accesso a \mgls{tracy};
	...
	\item premere il pulsante Create per confermare la creazione del nuovo \mgls{package};
\end{enumerate}
...
\subsubsubsection{Codifica}\label{code}

Nell'attività di codifica i \PRpl{}, seguendo quanto specificato nella \DP{}, dovranno produrre il codice sorgente del \mgls{software}.

\paragraph{Convenzioni e norme stilistiche}
\begin{itemize}
	\item Tutti i file che vengono creati contenenti codice devono essere codificati tramite \mgls{utf-8} e senza \mgls{bom};
	\item i programmatori devono prendere come riferimento le norme stilistiche fornite da \mgls{google}, specifiche per
	linguaggio adottato;
	\item i nomi di variabili, classi e metodi devono essere scritti in inglese;
	\item i nomi dei \mgls{package} devono avere l’iniziale maiuscola e utilizzare il CamelCase;
...
Il codice scritto per il \mgls{framework} \mgls{angularjs} dovrà seguire le seguenti linee guida:
\begin{itemize}
...
Il codice scritto in \mgls{javascript} dovrà seguire le seguenti linee guida:
\begin{itemize}
...
Il codice scritto in \mgls{html5} e \mgls{css3} dovrà seguire le seguenti linee guida:
\begin{itemize}
...
Per documentare il codice \mgls{javascript} si farà uso di Jsdoc. Ogni variabile di classe deve essere documentata quindi prima di ognuna dovrà essere presente un commento contenente:\\\\
\texttt{/**\\
...
Attraverso questo test si verifica la correttezza di tutti i moduli base che compongono i \mgls{software} andando a limitare gli errori di implementazione.
...
È un \mgls{framework} di test eseguito su \mgls{node.js}, supporta il lavoro con i browser, test asincroni, resoconti della copertura del prodotto con i test, e uso dei vari librerie di asserzioni.
\subparagraph{Installazione}
Per installare \mgls{mocha} si ha bisogno del \mgls{node package manager} (npm), quindi da terminale bisogna eseguire il commando:
\begin{center}
...
Jasmine, come Mocha \ref{strumentoTest}, è un \mgls{framework} di test per \mgls{javascript} che supporta il lavoro con i \mgls{browser}, test asincroni, resoconti della copertura del prodotto con i test. E' particolarmente adatto per testare i siti web, i progetti \mgls{node.js} e qualunque ambiente su cui possono essere eseguiti \mgls{javascript}. Data la possibilità di eseguire i test su svariati \mgls{browser} (Safari, Chrome, Firefox, e Internet Explorer) risulta essere particolarmente adatto allo sviluppo dei test per il \mgls{front-end} dell'applicazione.
\subparagraph{Installazione}
Per installare \mgls{jasmine} si ha bisogno del \mgls{node package manager} (npm), quindi da terminale bisogna eseguire il commando:
\begin{center}
	\texttt{\$ npm install -g jasmine}  
\end{center}
\subparagraph{Esecuzione}
Per eseguire i test \mgls{jasmine} ha bisogno del test runner, nel nostro caso Karma.
\begin{center}
\texttt{\$ karma start tests/client/karma.conf.js}  
\end{center}
\paragraph{Karma} \label{karma}
Karma è un test runner che può lavorare con diversi \mgls{framework} di test per \mgls{javascript} che supporta il lavoro con i \mgls{browser}, test asincroni, resoconti della copertura del prodotto con i test. E' particolarmente adatto per testare i siti web, i progetti \mgls{node.js} e qualunque ambiente su cui possono essere eseguiti \mgls{javascript}. Data la possibilità di eseguire i test in ambiente dei svariati \mgls{browser} (Safari, Chrome, Firefox, e Internet Explorer) risulta essere particolarmente adatto alla esecuzione dei test per il \mgls{front-end} dell'applicazione.
\subparagraph{Installazione}
Per installare jasmine si ha bisogno del \mgls{node package manager} (npm), quindi da terminale bisogna eseguire il commando:
\begin{center}
...
\mgls{protractor}  è un \mgls{framework} utilizzato per i test di sistema e validazione dell'applicazione sviluppata in \mgls{angularjs}. \mgls{protractor} permette di testare l'applicazione attraverso browser reali, interagendo come farebbe un utente vero.
...
\subparagraph{Installazione}
Per installare \mgls{protractor} si ha bisogno del \mgls{node package manager} (npm), quindi da terminale bisogna eseguire il commando:
\begin{center}
...
%\subsubsubsection{Validazione della progettazione}
%\TODO{}
%\subsubsubsection{Validazione dei diagrammi UML}
%\TODO{}
%\subsubsubsection{Validazione dei casi d'uso}
%\TODO{}
...
\newpage
\begin{center}
	\begin{longtable}[H]{p{0.2\textwidth} p{0.35\textwidth} p{0.15\textwidth} p{0.2\textwidth}}
		\toprule
