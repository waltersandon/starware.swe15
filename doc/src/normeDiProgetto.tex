...
\end{center}
Per eseguire i test \mgls{jasmine} ha bisogno di un test runner che nel nostro caso è Karma.
\subparagraph{Installazione Karma}
Per installare Karma si ha bisogno del \mgls{node package manager} (npm), quindi da terminale bisogna eseguire il commando:
\begin{center}
	\texttt{\$ npm install -g karma}  
\end{center}
\subparagraph{Esecuzione}
Per eseguire i test bisogna avviare il seguente comando trovandosi nella cartella \path{starware.swe15/src}:
\begin{center}
...
\end{center}
\subparagraph{Esecuzione}
Per eseguire i test bisogna eseguire una serie dei comandi al terminale trovandosi nella cartella \path{starware.swe15/src}: 
\begin{center}
