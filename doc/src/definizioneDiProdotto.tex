...
\mgls{express} è un \mgls{framework} minimale che aiuta ad organizzale le applicazioni web create con \mgls{node.js} secondo l'architettura \mgls{mvc} sul lato server. \mgls{express} viene installato come un modulo di \mgls{node.js} e lo integra, fornendo una migliore implementazione del sistema di \mgls{routing}, incrementando le funzioni di richiesta e risposta estendendole per una maggior flessibilità, integrando nuovi \mgls{middleware}, ed agevolando la realizzazione delle \mglspl{vista}. \mgls{express} aiuta l'utente a gestire le \mgls{route}, le \mgls{request} e le \mgls{view}.
\mgls{express},oltre che per il \mgls{routing} delle richieste e risposte, verrà utilizzato per definire l'ordine dei \mgls{middleware} nella \textit{chain of responsability} (vedere sezione Descrizione Design Pattern \ref{descrizioneDesignPattern}) ed in fine per restituire al \mgls{client} le risposte \mgls{http} con gli eventuali errori riscontrati dal \mgls{server}.
