...
\begin{itemize}
	...
	\item Permette di processare parallelamente i dati (\mgls{mapreduce})
...
\begin{itemize}
	\item Un \mgls{controller} può inviare comandi al \mgls{model} per aggiornare il suo stato. Può inoltre inviare comandi alla sua \mgls{view} associata, in modo da cambiarne la presentazione;
	\item Un \mgls{model} quando cambia il suo stato interno notifica le sue \mgls{view} e i suoi \mgls{controller} associati. Questo permette alle \mgls{view} di cambiare la loro presentazione e ai \mgls{controller} di cambiare il loro insieme di comandi disponibili;
	\item Una \mgls{view} viene aggiornata dal \mgls{controller} sui dati che necessita per generare un output per l’utente.
\end{itemize}
\TODO{teoricamente la view non viene aggiornata dal controller bensi si aggiorna ogni volta che avviene una modifica al model in quanto lo osserva, mentre il controller guarda solo la view senza mandarle comandi ma "ricevendone" solamente}
...
\subsection{Design Pattern Comportamentali}
\subsubsection{Chain of Responsibility}
Il Chain of Responsibility è un pattern comportamentale che permette di separare i \mgls{sender} dai \mgls{reciever} delle richieste. La richiesta attraversa una catena di oggetti per essere intercettata solo quando raggiunge il proprio gestore. Viene utilizzato quando non è possibile determinare staticamente il \mgls{reciever} oppure l’insieme di oggetti gestori cambia dinamicamente a runtime. Le richieste vengono dette implicite poiché il \mgls{sender} non ha alcuna conoscenza sull’identità del ricevente. Per permettere alla richiesta di attraversare la catena e per rimanere implicita, ogni \mgls{reciever} condivide un interfaccia comune per gestire le richieste ed accedere al proprio successore. La gerarchia che vorrà inviare richieste dovrà avere una superclasse che dichiara un metodo handler generico. La specializzazione di tale metodo avviene tramite overriding nelle sottoclassi opportune.\\
