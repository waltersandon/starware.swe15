...
\mgls{express} è un \mgls{framework} minimale che aiuta ad organizzare le applicazioni web create con \mgls{node.js} secondo l'architettura \mgls{mvc} sul lato server.
...
Async è un modulo di utilità che fornisce funzioni per lavorare in modo asincrono con \mgls{javascript}. Questo modulo è stato originariamente sviluppato per funzionare con \mgls{node.js} ed è installabile attraverso il \mgls{node package manager}, inoltre è utilizzabile direttamente tramite \mgls{browser}.
Async fornisce circa una settantina di funzionalità (map, reduce, filter, parallel, series, waterfall, ...). Tutte queste funzionalità per funzionare correttamente devono essere utilizzate assumendo di seguire le convenzioni di \mgls{node.js} (es. bisogna fornire una singola callback come ultimo argomento della funziona asincrona).
...
SimpleMDE markdown editor è una libreria che provvede a fornire un editor \mgls{markdown} semplice da usare e comodo per gli utenti. Si integra perfettamente con \mgls{javascript} e permette di avere un editor \mgls{wysiwyg}.
