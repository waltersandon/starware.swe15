...
I documenti verranno distribuiti in formato \mgls{pdf}. Per la stesura dei documenti verrà utilizzato \LaTeX{} come \emph{word processor} e come linguaggio di markup. Verranno utilizzati i seguenti software per la produzione di materiale per i documenti:
\begin{itemize}
  ...
  \item \textbf{ArgoUML\footnote{\url{http://argouml.tigris.org/}}}: (referente \AVE) verrà utilizzato come software per creare tutti i diagrammi \mgls{uml}.
\end{itemize}
Tutto il software utilizzato è gratuito e \emph{\mgls{cross-platform}}.
...
Durante l'applicazione del \textit{\mgls{walkthrough}} ai documenti, sono state riportate le tipologie di errori più frequenti. La lista
...
\begin{itemize}
  \item \textbf{Ticket:} il Verificatore apre un \mgls{ticket} su Asana per la risoluzione del problema riscontrato (dettagli nella sezione \TODO{});
  ...
  \item \textbf{Correzione immediata:} il \VR{} provvede autonomamente alla correzione dell'errore, segnalando la modifica effettuata tramite sistema di \mgls{versionamento}, indicando un messaggio di \mgls{commit} esaustivo (per approfondimenti si rimanda alla sezione sulle norme di \mgls{versionamento} \TODO{}).
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Verifica delle proprietà di glossario:} Nella redazione del glossario è possibile automatizzare tale verifica. Utilizzando l'apposito script, vengono individuate tutte le segnalazioni di una parola del glossario in un documento in maniera automatica, ed applicato l'opportuno stile. Tale script è descritto nella sezione \TODO{} Il Verificatore dovrà quindi controllare che l'output finale sia conforme a quanto precisato relativamente al glossario
	\item \textbf{Miglioramento del processo di verifica:} quando i Verificatori eseguono \emph{\mgls{walkthrough}} di un documento, dovranno riportare gli errori più frequentemente trovati. Grazie a tale pratica sarà possibile eseguire \emph{\mgls{inspection}} su tali errori nelle verifiche future
	\item Norme stilistiche \TODO{}
	\item \textbf{Segnalazione degli errori riscontrati:} Attenendosi a quanto riportato nella tabella \ref{tabCalssificazioneProb} il \VR{} deve generare ticket secondo quando descritto nella sezione \TODO{}
\end{itemize}

\subsubsection{Diagrammi UML}
Al \VR{} è richiesto il controllo dei diagrammi UML prodotti:
\begin{itemize}
	\item \textbf{Diagrammi di caso d'uso:} Il controllo dei diagrammi di caso d'uso deve avvenire manualmente, controllando il rispetto delle specifiche \mgls{uml} 2.4 e il corretto uso delle relazioni di inclusione ed estensione. Il diagramma di caso d'uso deve rappresentare fedelmente quanto descritto dal caso d'uso.
\end{itemize}
...
I processi di management contengono le attività generiche e i task, che possono essere impiegati da un qualsiasi gruppo che deve gestire i suoi rispettivi processi. Il \PM{} responsabile per la gestione del prodotto, la gestione del progetto, la gestione dei \textit{\mglspl{task}} inerenti ai processi come quelli di acquisizione, fornitura, sviluppo, manutenzione o i processi di supporto.
...
\begin{itemize}
  \item Programmazione del lavoro per il completamento tempestivo dei \mgls{task};
  \item Stima dello sforzo necessario;
  \item Assegnazione delle risorse necessarie al completamento del \mgls{task};
  \item Assegnazione delle responsabilità;
  \item Quantificazione dei rischi associati ai \mgls{task};
...
Per pianificare le attività il \PM{} deve realizzare un diagramma di \mGls{gantt} per ciascuna fase indicata nella sezione Pianificazione del \PdP{}.
Per assolvere a questo compito è stato scelto il software open-source \mGls{projectlibre} per i seguenti motivi:
\begin{itemize}
  \item permette di creare diagrammi di \mGls{gantt} e \mGls{PERT} in maniera facile ed intuitiva;
...
Per la suddivisione e l'assegnazione dei \textit{\mgls{task}} è stato scelto di utilizzare l'applicazione web \textit{Asana} per i seguenti motivi:
 \begin{itemize}
  \item Permette di creare e assegnare \textit{\mgls{task}} in maniera facile ed intuitiva;
  \item Permette di monitorare chiaramente le scadenze dei \textit{\mgls{task}};
  \item Da la possibilità dividere i \textit{\mgls{task}} in \textit{\mgls{subtask}};
  \item Esiste anche un applicazione per \textit{smartphone} rendendo molto più accessibile l'utilizzo di \textit{Asana}.
\end{itemize}
Sarà inoltre compito del \PM{} assegnare una deadline per il completamento del \textit{\mgls{task}} e risolvere eventuali problemi legati a ritardi nel completamento degli stessi.
...
Verranno definite norme riguardanti:
\begin{itemize}
  \item Definizione e documentazione sulle modalità di \textit{\mgls{ticketing}} ;
  \item Definizione e documentazione sulle modalità di \textit{\mgls{versionamento}};
  \item Definizione e documentazione sulla struttura del sistema di \textit{\mgls{repository}}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Assegnatari/o:} I \textit{\mglspl{task}} o \textit{\mglspl{subtask}} devono sempre essere assegnati ad uno o più membri del gruppo;
  \item \textbf{Data:} Ogni \textit{\mglspl{task}} o \textit{\mglspl{subtask}} deve sempre avere una data, decisa attraverso i diagrammi di \textit{\mgls{gantt}} e \textit{\mgls{PERT}}, entro il quale essere completato;
  ...
  \item \textbf{Label:} Bisogna segnalare lo stato del \textit{\mgls{task}} attraverso una etichetta appropriata;
  \item \textbf{Subtask:} In caso di necessità è possibile suddividere il \textit{\mgls{task}} in diversi \textit{\mgls{subtask}} imputabili ai membri del gruppo.
\end{itemize}

\newpage
\subsubsubsection{Etichette}
Lo stato di ogni \textit{\mgls{ticket}} deve essere identificato da un appropriata etichetta:
\begin{itemize}
  \item \textbf{Aperto:} Etichetta riservata ai \textit{\mgls{ticket}} non ancora assegnati;
  \item \textbf{Accettato:} Etichetta riservata ai \textit{\mgls{ticket}} una volta in cui il membro del gruppo a cui sono imputati ne ha preso visione;
  \item \textbf{In Corso:} Etichetta riservata ai \textit{\mgls{ticket}} in fase di elaborazione;
  \item \textbf{In Attesa:} Etichetta riservata ai \textit{\mgls{ticket}} che non possono entrare in fase di elaborazione a causa di dipendenze con altri ticket non ancora completati;
  \item \textbf{Completato:} Etichetta riservata ai \textit{\mgls{ticket}} per cui la fase di elaborazione è stata completata con successo;
  \item \textbf{Verificato:} Etichetta riservata ai \textit{\mgls{ticket}} di cui è stata verificato l'effettivo comletamento e l'assenza di errori;
\end{itemize}

\subsubsubsection{Ciclo di vita del Ticket}
Ogni \textit{\mgls{ticket}} rappresenta un \mgls{attivita} o un \textit{\mgls{task}} e deve rispettare il seguente iter:

\begin{enumerate}
  \item Creazione di un \textit{\mgls{task}};
  \item Il \PM{} assegna il \textit{\mgls{task}} ad uno o più membri del gruppo, nel caso l'assegnatario non ne abbia ancora preso visione o non abbia ancora deciso se accettare il \textit{\mgls{task}}, sarà necessario impostare come etichetta \textbf{Aperto};  
    \item L'assegnatario può decidere se accettare o meno il \textit{\mgls{task}}:
    	\begin{itemize}
    	\item Accettazione: L'assegnatario dovrà modificare l'etichetta del \textit{\mgls{task}} in \textbf{Accettato}.
    	\item Rifiuto: L'assegnatario dovrà motivare il rifiuto del \textit{\mgls{task}} e, nel caso le motivazione fossero reputate pertinenti dal \PM{}, verrà ripetuto il punto 2. 
    	\end{itemize}
	\item Se il \textit{\mgls{task}} fosse dipendente dal completamento di un altro \textit{\mgls{task}} in stato di esecuzione, sarà necessario modificarne l'etichetta in \textbf{In attesa};
	\item Esecuzione del \textit{\mgls{task}};
	\item Una volta completato il \textit{\mgls{task}} l'assegnatario dovrà modificarne l'etichetta in \textbf{Completato};
...
\centering
\label{f1-Ticket}
\includegraphics[scale=0.5]{ticketing.png}
...
Per ogni \textit{\mgls{task}} marcato da etichetta \textbf{Completato} deve esserne verificata la correttezza e l'effettivo completamento attraverso la seguente procedura:
\begin{enumerate}
	\item Al completamento di un \textit{\mgls{ticket}} il \PM{} deve creare un \textit{\mgls{task}} marcato dall'etichetta \textbf{Verificare} ed assegnarlo ad un \VR{};
	\item Il \VR{}, nel caso riscontri anomalie o errori deve creare i \textit{\mgls{task}} necessari alla correzione degli stessi. Nel caso dell'assenza di errori il \VR{} chiude il \textit{\mgls{task}} e ne modifica lo stato impostando l'etichetta \textbf{Verificato}.
	\item Il \PM{} assegna gli eventuali \textit{\mgls{task}} creati dal \VR{} ponendo particolare attenzione a non far sorgere conflitti di interesse.
\end{enumerate}
