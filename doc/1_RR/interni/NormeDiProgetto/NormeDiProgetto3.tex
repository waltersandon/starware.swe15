...
\begin{itemize}
	...
	\item \textbf{Verifica del glossario:} La redazione del glossario è automatizzata. Il \VR{} dovrà quindi solo controllare che tutte le voci del glossario abbiano una definizione e che tutte le parole che sono definite nel glossario siano marcate con una G a pedice \TODO{tool di controllo}
	...
	\item \textbf{Segnalazione degli errori riscontrati:} Attenendosi a quanto riportato nella tabella \ref{tabCalssificazioneProb} il \VR{} deve generare dei \mglspl{ticket} secondo quando descritto nella sezione \ref{ticket}
\end{itemize}
...
I processi di amministrazione contengono le attività generiche e i task, che possono essere impiegati da un qualsiasi gruppo che deve gestire i suoi rispettivi processi. Il \PM{} responsabile per la gestione del prodotto, la gestione del progetto, la gestione dei \mglspl{task} inerenti ai processi come quelli di acquisizione, fornitura, sviluppo, manutenzione o i processi di supporto. Verranno definite norme riguardanti:
\begin{itemize}
...
Ogni verbale redatto dovrà essere inserito nell'apposita cartella \filePath{verbali} nei \mgls{repository} e dovrà essere nominato \fileName{verbale\_data}, dove la data seve seguire il formato indicato nella sezione \ref{formati} di questo documento.
...
\begin{enumerate}
	...
	\item inserire i periodi di \mgls{slack} dove previsto;
...
\mgls{projectlibre} viene anche utilizzato come strumento di controllo in quanto permette di calcolare automaticamente le metriche di \mGls{schedule variance} e \mGls{cost variance}. Per far ciò il Responsabile deve modificare i progetti creati nel modo seguente:
\begin{enumerate}
...
Le modalità di utilizzo di \mgls{teamwork} vengono descritte nella sezione [ticket]{6.5} di questo documento.

\subsubsection{Ticketing}\label{ticket} %6.5 delle NdPv5.2.0 dei don't panic
Per monitorare lo sviluppo dei documenti e del codice si è scelto di adottare il sistema di ticketing di \mGls{teamwork}. La scelta di tale software è descritta nella sezione [swGestioneProgetto]{6.2.2} .
...
\begin{itemize}
	...
	\item \textbf{Data:} Ogni \mglspl{task} deve sempre avere una data, decisa attraverso i diagrammi di \mGls{gantt} e \mgls{PERT}, entro il quale essere completato;
	...
	\item \textbf{Etichetta:} Deve essere fornito di un appropriata \mgls{etichetta} per aiutare visivamente i membri del gruppo a distinguere di che tipologia di \mgls{ticket} si tratta. 
...
\begin{itemize}
	\item \textbf{Error:} riservata ai \mglspl{ticket} per la segnalazione di  un errori nei documenti o un errore logico nell'implementazione del codice;
	\item \textbf{Bug:} riservata ai \mglspl{ticket} per la segnalazione di \mgls{bug} nel codice;
	\item \textbf{Missing Feature:} riservata ai \mglspl{ticket} per la segnalazione la mancanza di una funzionalità del software;
	\item \textbf{Code:} riservata ai \mglspl{ticket} per la produzione di codice;
	\item \textbf{Document:} riservata ai \mglspl{ticket} per la produzione di documentazione;
	\item \textbf{Check:} riservata ai \mglspl{ticket} di verifica.
\end{itemize}

\subsubsubsection{Ciclo di vita del Ticket}\label{ticketCicloVita}
Ogni \mgls{ticket} rappresenta un \mgls{attivita} o un \mgls{task} e deve rispettare i seguenti passi:
\begin{enumerate}
	\item Creazione di un \mgls{task};
	\item Il \PM{} assegna il \mgls{task} ad uno o più membri del gruppo, nel caso l'assegnatari/o non ne abbia ancora preso visione o non abbia ancora deciso se accettare il \mgls{task}, lo stato del \mgls{ticket} sarà \textbf{"Aperto"};
	...
	\item Una volta termiata l'esecuzione del \mgls{task}, lo stato sarà \textbf{"Comletato"};
	\item Ogni \mgls{task} completato dovrà essere vagliato attraverso le prodecure di verifica.
\end{enumerate}
...
Ogni \mgls{task} in stato \textbf{Completato} deve essere verificato per testarne la correttezza e l'effettivo completamento attraverso la seguente procedura:

\begin{enumerate}
	\item Al completamento di un \mgls{ticket} il \PM{} deve creare un \mgls{task} in stato Verifica, marcato dall'\mgls{etichetta} \textbf{Check} ed assegnarlo ad un \VR{};
	\item Il \VR{}, nel caso riscontri anomalie o errori deve creare i \mgls{task} necessari alla correzione degli stessi. Nel caso dell'assenza di errori il \VR{} chiude il \mgls{task} che avrà raggiunto lo stato \textbf{"Verificato"};
	\item Il \PM{} assegna gli eventuali \mgls{task} creati dal \VR{} ponendo particolare attenzione a non far sorgere conflitti di interesse tra i vari ruoli.
\end{enumerate}
