...
Come \mgls{software} per il tracciamento dei requisiti si è scelto di utilizzare \mGls{tracy}, realizzato dal gruppo Don’t Panic durante l’anno accademico 2012/2013.
...
Le caratteristiche che hanno fatto propendere il gruppo ad usare questo \mgls{software} sono state un interfaccia elementare ed intuitiva, il supporto di \mgls{uml}2.0 e la possibilità di utilizzare questa applicazione sia da \mgls{browser} che da \mGls{desktop}.
...
\begin{itemize}
	\item \textbf{Ticket:} il Verificatore apre un \mgls{ticket} su \textit{\mGls{teamwork}} per la risoluzione del problema riscontrato (dettagli nella sezione \ref{ticket});
...
I processi di amministrazione contengono le attività generiche e i task, che possono essere impiegati da un qualsiasi gruppo che deve gestire i suoi rispettivi processi. Il \PM{} responsabile per la gestione del prodotto, la gestione del progetto, la gestione dei \textit{\mglspl{task}} inerenti ai processi come quelli di acquisizione, fornitura, sviluppo, manutenzione o i processi di supporto. Verranno definite norme riguardanti:
\begin{itemize}
...
Il \PM{} deve convocare la riunione attraverso la creazione di un \textit{\mgls{ticket}} su \textit{\mGls{teamwork}} avente le caratteristiche descritte nella sezione \ref{ticketRiunione} di questo documento.
...
Ogni verbale redatto dovrà essere inserito nell'apposita cartella \filePath{verbali} nei \textit{\mgls{repository}} e dovrà essere nominato \fileName{verbale\_data}, dove la data seve seguire il formato indicato nella sezione \ref{formati} di questo documento.
...
\begin{itemize}
	...
	\item Offre la possibilità di impostare delle \textit{\mglspl{milestone}} per tenere facilmente sotto controllo lo svolgimento delle \mgls{attivita}.
\end{itemize}
...
Le modalità di utilizzo di \textit{\mgls{teamwork}} vengono descritte nella sezione [ticket]{6.5} di questo documento.

\subsubsection{Ticketing}\label{ticket} %6.5 delle NdPv5.2.0 dei don't panic
Per monitorare lo sviluppo dei documenti e del codice si è scelto di adottare il sistema di ticketing di \textit{\mGls{teamwork}}. La scelta di tale software è descritta nella sezione [swGestioneProgetto]{6.2.2} .
...
\begin{itemize}
	...
	Le \mGlspl{etichetta} utilizzate sono descritte nella sezione  [ticketEtichette]{6.5.1.2}.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Check:} riservata ai \textit{\mglspl{ticket}} di verifica.
\end{itemize}
...
\begin{enumerate}
	\item Al completamento di un \textit{\mgls{ticket}} il \PM{} deve creare un \textit{\mgls{task}} in stato Verifica, marcato dall'\mgls{etichetta} \textbf{Check} ed assegnarlo ad un \VR{};
