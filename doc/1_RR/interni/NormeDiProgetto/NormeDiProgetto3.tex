...
Ogni caso d'uso deve avere la seguente struttura:
\begin{itemize}
	\item \textbf{Identificativo:} ogni caso d'uso deve essere identificato in modo univoco. L'identificativo dovrà avere la seguente forma \textbf{"UC\#"} dove il carattere \textbf{"\#"} rappresenta un numero incrementale.
	\item \textbf{Titolo:} Un titolo breve, sintetico e descrittivo
	\item \textbf{Pre-condizione:} descrizione sintetica della condizione preliminare rispetto alla possibilità di realizione del caso d'uso
	\item \textbf{Post-condizione:}	descrizione sintetica della condizione che deve essere vera al verificarsi del caso d'uso
\end{itemize}
...
Ogni requisito deve avere la seguente struttura:
\begin{itemize}
	\item \textbf{Identificativo:} ogni requisito deve essere identificato in modo univoco.
	\item \textbf{Categoria:} identifica la categoria del requisito: "Funzionale" , "Qualità", "Prestazionale" o "Vincolo"
	\item \textbf{Priorità:} identifica la priorità del requisito: "Obbligatorio", "Opzionale" o "Desiderabile"
	\item \textbf{Descrizione:}	descrizione sintetica del requisito
	\item \textbf{Validazione:}	\TODO{}
	\item \textbf{Test:} \TODO{}
...
\begin{itemize}
	\item \textbf{Corsivo:} deve essere usato solo per indicare riferimenti agli standard, citazioni, \mgls{ruoli} e nomi di documenti
...
\begin{itemize}
	...
	\end{itemize}
	\item \textbf{Attività:} sono stati definite macro  \LaTeX per i documenti in modo da facilitare la redazione dei documenti per i riferimenti alle attività del \PdP: \TODO{sistemare se cambiano i nomi}
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Calcolo dell’indice \mgls{gulpease}:} Su ogni documento redatto il \VR{} deve calcolare l’indice di leggibilità utilizzando lo strumento automatico fornito nel \mgls{makefile}. Nel caso in cui l’indice risultasse troppo basso, sarà necessario eseguire un \mgls{walkthrough} del documento alla ricerca delle frasi troppo lunghe o complesse;
...
Quando il \RE{} prende visone di un \mgls{ticket} di segnalazione problema provende all'immediata assegnazione di un \mgls{ticket} di risoluzione problema che ha la seguente struttura:
...
\mgls{projectlibre} viene anche utilizzato come strumento di controllo in quanto permette di calcolare automaticamente le metriche di \mGls{schedule variance} e \mGls{cost variance}. Per far ciò il \RE deve modificare i progetti creati nel modo seguente:
\begin{enumerate}
...
Ogni \mgls{ticket} rappresenta un \mgls{task} o un'insieme di \mglspl{task} ( \mgls{attivita}) e deve rispettare i seguenti passi:
\begin{enumerate}
...
\begin{itemize}
	...
	%\item la \mgls{checkbox} \mgls{appointed} deve essere spuntata solo nei requisiti opzionali e desiderabili presi in carico;
