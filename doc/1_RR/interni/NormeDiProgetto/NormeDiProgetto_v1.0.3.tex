...
\begin{itemize}
	\item \textbf{Interno:} i docuemnti definiti ad uso interno non devono essere distribuiti all'esterno del gruppo di lavoro.
	\item \textbf{Esterno:} i docuemnti definiti ad uso esterno devono essere distribuiti secondo i criteri della lista di distribuzione solamente se sono stati approvati dal \PM{} e se sono in stato \gloss{formale}.  
\end{itemize}
...
Il \PM{} deve convocare la riunione attraverso l'invio di una \textit{\gloss{email}} a tutti i membri del gruppo, avente le seguenti caratteristiche:
\begin{itemize}
...
Ogni verbale redatto dovrà essere inserito all'interno della apposita cartella \fileName{verbali} creata nel \textit{\gloss{repository}} su \textit{\gloss{GitHub}} e dovrà essere nominato \fileName{verbale\_data}, la data seve seguire il formato indicato nella sezione 3.3.5.3.

\paragraph{Riunione interna}
Il verbale di riunione interna è un documento interno informale che consente di tracciare gli argomenti discussi durante la riunione. Verrà redatto dal segretario della riunione, ruolo scelto di volta in volta a rotazione tra i presenti.
...
Per la suddivisione e l'assegnazione dei \textit{\gloss{task}} è stato scelto di utilizzare il sistema di \textit{\gloss{issue}} di \textit{\gloss{GitHub}} per i seguenti motivi:
 \begin{itemize}
  \item Permette di creare e assegnare \textit{\gloss{task}} in maniera facile ed intuitiva;
  \item Permette di monitorare chiaramente le scadenze dei \textit{\gloss{task}} attraverso la creazione di \textit{\gloss{milestone}};
  \item Fornisce strumenti per mettere in relazione documenti e/o codice con i \textit{\gloss{tasks}}, dato che \textit{\gloss{GitHub}} è il sistema su cui si è scelto di mantenere i \textit{\gloss{repository}}.
\end{itemize}
Sarà inoltre compito del \PM{} assegnare una deadline per il completamento dei \textit{\gloss{tasks}} e risolvere eventuali problemi legati a ritardi nel completamento degli stessi.
...
Viene creata una \gloss{repository} su github all'indirizzo \url{https://github.com/lordnikolai/starware.swe15}. Ogni membro deve clonare la repository in locale tramite il comando \texttt{git clone}.
