...
Per le comunicazioni interne informali è stato scelto l'uso di \textit{\mgls{telegram}}, per il suo utilizzo tramite \textit{\mgls{browser}} che permette, quindi, un accesso alle conversazioni sia da pc che da \textit{smartphone}. Per lo scambio e la condivisione di file e appunti è invece utilizzato il servizio \textit{cloud} \mgls{Google Drive}. Inoltre è permesso l'uso di \mgls{skype} per effettuare conferenze di gruppo o anche tra singoli membri di esso.
...
Ogni verbale redatto dovrà essere inserito nell'apposita cartella \filePath{verbali} nei \textit{\mgls{repository}} e dovrà essere nominato \fileName{verbale\_data}, dove la data seve seguire il formato indicato nella sezione \hyperref[formati]{3.7.3} di questo documento .
...
\end{center}
Per una guida dettagliata all'uso della \mgls{repository} si veda la sezione \ref{sec:rep} .
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Responsabile}: \latex{\textbackslash RE}
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Progettazione architetturale}: \latex{\textbackslash FPA}
		\item \textbf{Progettazione di dettaglio e codifica}: \latex{\textbackslash FPDC}
		\item \textbf{Verifica e validazione}: \latex{\textbackslash FVV}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \textbf{Analisi statica:} tale tecnica è attuabile sia alla documentazione sia al codice, consiste nell'individuazione di errori ed anomalie ad esempio effettuando una lettura critica del testo a largo spettro oppure più mirata. Il \VR{} controllerà i documenti e il codice utilizzando le seguenti tecniche:
    \begin{itemize}
      \item \textbf{Walkthrough:} consiste in una lettura a largo spettro del documento/codice in questione che permetta di individuare i possibili errori o anomalie presenti. Con questa tecnica non si conosce la tipologia o l'insieme di errori da ricercare, bensì ogni aspetto è attentamente analizzato al fine di ricavare, successivamente, delle categorie di errori tipici. Ogni difetto rilevato è discusso con gli autori allo scopo di evitare incomprensioni e concordare le modifiche necessarie. Il \textit{\mgls{walkthrough}} è indispensabile nelle prime fasi di sviluppo quando non si possiede ancora una chiara visione dei possibili errori. Usando più volte questa tecnica è possibile stilare una \mgls{lista di controllo} dove vengono archiviati gli errori più spesso rilevati. Man mano che il \VR{} rende più esaustiva la \mgls{lista di controllo}, si ricorre sempre meno all'utilizzo della tecnica di \textit{\mgls{walkthrough}} a favore della tecnica di \textit{\mgls{inspection}};
      \item \textbf{Inspection:} l'\textit{\mgls{inspection}} si basa sulla lettura mirata dei documenti/codice. Durante tale lettura si cercano gli errori segnalati nella \mgls{lista di controllo}. Progressivamente, con l'acquisizione di esperienza e grazie al precedente uso della tecnica di \textit{\mgls{walkthrough}}, la \mgls{lista di controllo} viene estesa e specializzata, rendendo l'\textit{\mgls{inspection}} sempre più efficace;
        \end{itemize}
\end{itemize}

\subsubsection{Classificazione dei problemi}
A corredo della \mgls{lista di controllo}, vengono classificate le categorie delle problematiche più comuni (si veda tabella \ref{tabCalssificazioneProb}) e le giuste tecniche di risoluzione, adattate alla gravità dei relativi errori. Essi si dividono per ambito e si ordinano secondo una priorità dettata dall'urgenza di risoluzione. Ambiti:
\begin{itemize}
  \item Processi;
  \item Documentazione;
  \item Codice.
...
\begin{itemize}
  \item \textbf{Ticket:} il Verificatore apre un \mgls{ticket} su Asana per la risoluzione del problema riscontrato (dettagli nella sezione \ref{ticket});
...
\begin{itemize}
	...
	\item \textbf{Miglioramento del processo di verifica:} quando i \VRpl{} eseguono \emph{\mgls{walkthrough}} di un documento, dovranno riportare gli errori più frequentemente trovati. Grazie a tale pratica sarà possibile eseguire \emph{\mgls{inspection}} su tali errori nelle verifiche future
	\item \textbf{Segnalazione degli errori riscontrati:} Attenendosi a quanto riportato nella tabella \ref{tabCalssificazioneProb} il \VR{} deve generare dei \textit{\mglspl{ticket}} secondo quando descritto nella sezione \ref{ticket}
\end{itemize}
...
Viene creata una \mgls{repository} su github all'indirizzo \url{https://github.com/lordnikolai/starware.swe15}. Ogni membro deve clonare la repository in locale.
...
\paragraph{Branch} vengono definiti 3 \textit{branches} all'interno della \textit{\mgls{repository}}:
\begin{itemize}
...
\mgls{projectlibre} è un programma \textit{\mgls{open-source}} per il \textit{\mgls{project management}}. Tale software è stato scelto in quanto possiede le seguenti caratteristiche:
\begin{itemize}
...
Il \PM{}, una volta individuate e stimate le \mgls{attivita} e i \textit{\mgls{task}} da svolgere, ha la responsabilità di assegnarne l'esecuzione ai vari membri del gruppo. Per la suddivisione e l'assegnazione dei \textit{\mgls{task}} è stato scelto di utilizzare l'applicazione web \mGls{Teamwork} per i seguenti motivi: 
\begin{itemize}
	\item Fornisce un sistema flessibile per la gestione dei \textit{\mglspl{ticket}}
	...
	\item Offre la possibilità di impostare delle \textit{\mglspl{milestone}} per tenere facilente sotto controllo lo svoglimento delle \mgls{attivita}.
\end{itemize}
...
Le modalità di utilizzo di \textit{\mgls{Teamwork}} vengono descritte nella sezione \hyperref[ticket]{6.5} di questo documento.
...
Per la modellazione dei diagrammi dei caso d’uso, diagrammi di sequenza e diagrammi di attività è stato scelto l’editor \textit{\mGls{draw.io}}.
Le caratteristiche che hanno fatto propendere il gruppo ad usare questo \mgls{software} sono state un interfaccia elementare ed intuitiva, il supporto di \mgls{uml}2 e la possibilità di utilizzare questa applicazione sia da \mgls{browser} sia come applicazione \mGls{desktop}.
...
Per monitorare lo sviluppo dei documenti e del codice si è scelto di adottare il sistema di ticketing di \textit{\mGls{Teamwork}}. La scelta di tale software è descritta nella sezione \hyperref[swGestioneProgetto]{6.2.2} .

\subsubsubsection{Struttura Ticket}\label{ticketStruttura}
I \textit{\mgls{ticket}} creati devono avere le seguenti caratteristiche:
\begin{itemize}
	\item \textbf{Titolo:} Rappresenta in maniera sintetica e significativa il \textit{\mgls{task}};
	\item \textbf{Assegnatari/o:} I \textit{\mglspl{task}} devono sempre essere assegnati ad uno o più membri del gruppo;
	\item \textbf{Data:} Ogni \textit{\mglspl{task}} deve sempre avere una data, decisa attraverso i diagrammi di \mGls{gantt} e \mgls{PERT}, entro il quale essere completato;
	\item \textbf{Commento:} Nel caso fossero necessarie delucidazioni al \textit{\mgls{task}} bisogna chiarirle in maniera concisa sotto forma di commento;
	\item \textbf{Etichetta:} Deve essere fornito di un appropriata \mgls{etichetta} per aiutare visivamente i membri del gruppo a distinguere di che tipologia di \textit{\mgls{ticket}} si tratta. 
	Le \mGlspl{etichetta} utilizzate sono descritte nella sezione  \hyperref[ticketEtichette]{6.5.1.2}.
\end{itemize}


\subsubsubsection{Etichette}\label{ticketEtichette}
Le \mglspl{etichetta} servono per identificare meglio il \textit{\mgls{ticket}} creato, quelle in uso dal gruppo sono:
\begin{itemize}
	\item \textbf{Error:} riservata ai \textit{\mglspl{ticket}} per la segnalazione di  un errori nei documenti o un errore logico nell'implementazione del codice;
	\item \textbf{Bug:} riservata ai \textit{\mglspl{ticket}} per la segnalazione di \mgls{bug} nel codice;
	\item \textbf{Missing Feature:} riservata ai \textit{\mglspl{ticket}} per la segnalazione la mancanza di una funzionalità del software;
	\item \textbf{Code:} riservata ai \textit{\mglspl{ticket}} per la produzione di codice;
	\item \textbf{Document:} riservata ai \textit{\mglspl{ticket}} per la produzione di documentazione;
	\item \textbf{Check:} riservata ai \textit{\mglspl{ticket}} in fase di verifica;
\end{itemize}

\subsubsubsection{Ciclo di vita del Ticket}\label{ticketCicloVita}
Ogni \textit{\mgls{ticket}} rappresenta un \mgls{attivita} o un \textit{\mgls{task}} e deve rispettare i seguenti passi:
\begin{enumerate}
	\item Creazione di un \textit{\mgls{task}};
	\item Il \PM{} assegna il \textit{\mgls{task}} ad uno o più membri del gruppo, nel caso l'assegnatari/o non ne abbia ancora preso visione o non abbia ancora deciso se accettare il \textit{\mgls{task}}, lo stato del \textit{\mgls{ticket}} sarà \textbf{"Aperto"};
	\item L'assegnatario può decidere se accettare o meno il \textit{\mgls{task}}: 
	\begin{itemize}
		\item Accettazione: lo stato del \textit{\mgls{task}} sarà \textbf{"Accettato"};
		\item Rifiuto: L'assegnatario dovrà motivare il rifiuto del \textit{\mgls{task}} e, nel caso le motivazione fossero ritenute pertinenti dal \PM{}, verrà ripetuto il punto 2.
	\end{itemize}
	\item Se il \textit{\mgls{task}} fosse dipendente dal completamento di un altro \textit{\mgls{task}} in stato di esecuzione, allora lo stato del \textit{\mgls{task}} sarà \textbf{"In Attesa"};
	\item Si procede con l'esecuzione del \textit{\mgls{task}}, il cui stato passerà a \textbf{"In Esecuzione"};
	\item Una volta termiata l'esecuzione del \textit{\mgls{task}}, lo stato sarà \textbf{"Comletato"};
	\item Ogni \textit{\mgls{task}} completato dovrà essere vagliato attraverso le prodecure di verifica.
\end{enumerate}
...
\subsubsubsection{Ticket di Verifica}\label{ticketVerifica}
Ogni \textit{\mgls{task}} in stato \textbf{Completato} deve essere verificato per testarne la correttezza e l'effettivo completamento attraverso la seguente procedura:

\begin{enumerate}
	\item Al completamento di un \textit{\mgls{ticket}} il \PM{} deve creare un \textit{\mgls{task}} in stato Verifica ed assegnarlo ad un \VR{};
	\item Il \VR{}, nel caso riscontri anomalie o errori deve creare i \textit{\mgls{task}} necessari alla correzione degli stessi. Nel caso dell'assenza di errori il \VR{} chiude il \textit{\mgls{task}} che avrà raggiunto lo stato \textbf{"Verificato"};
	\item Il \PM{} assegna gli eventuali \textit{\mgls{task}} creati dal \VR{} ponendo particolare attenzione a non far sorgere conflitti di interesse tra i vari ruoli.
\end{enumerate}
...
Un \mgls{ticket} di riunione è creato attraverso l'impostazione di un \mgls{evento} su \textit{\mGls{Teamwork}} e deve avere le seguenti caratteristiche:
 \begin{itemize}
 	...
 	\item \textbf{Ordine del Giorno:} elenco ordinato delle varie voci da esaminare sotto forma di commento al \textit{\mgls{ticket}};
