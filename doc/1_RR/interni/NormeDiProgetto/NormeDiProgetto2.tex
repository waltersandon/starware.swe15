...
Per la condivisione informale di file e per il lavoro collaborativo su documenti di supporto, si usa la piattaforma di condivisione file online \mgls{Google Drive}. Trattandosi di strumenti informali, non si definiscono procedure rigorose d’uso e se ne lascia la descrizione alle sezioni \TODO{}.
...
\paragraph{Dropbox} che è una piattaforma di condivisione dove devono essere caricati solo file che non necessitano di controllo di versione e che sono comunque recuperabili da altre fonti (es. internet). Si caricheranno qui i file di installazione dei software utilizzati dal gruppo in modo da uniformare le versioni utilizzate da ogni componente. Vengono resi disponibili anche i manuali (software, libri, PDFG ) di consultazione così da avere uniformità di versione e di informazione. Altresì è presente una cartella dove il server esegue il salvataggio dei backup del database. La possibilità di installare Dropbox sul proprio PC dà a ogni componente del gruppo la possibilità di avere a disposizione documentazione e software anche offline.
...
ProjectLibre è un programma open source per il project management. É basato su Java ed è quindi eseguibile su ogni sistema operativo. É il successore ufficiale di OpenProj. Tale software è stato scelto in quanto possiede le seguenti caratteristiche:
\begin{itemize}
	...
	\item genera automaticamente la Work Breakdown StructureG (WBS), a partire dal Gantt con allocazione di risorse;
	\item calcola i parametri Schedule Variance (SV) e Budget Variance (BV);
	\item compatibile con Microsoft Project 2010;
	\item salvataggio su file XML: essendo un file testuale è possibile effettuare il merge	di più file in caso di conflitti sul repository.
\end{itemize}
...
ProjectLibre viene anche utilizzato come strumento di controllo in quanto permette di calcolare automaticamente le metriche di Budget Variance e di Schedule Variance. Per far ciò il Responsabile deve modificare i progetti creati nel modo seguente:
\begin{enumerate}
